...
def _get_aux_images(sample):
    return [sample['ref_aux_img'], sample['prev_aux_img']] + sample['curr_aux_img']
...
def _merge_sample_with_aux(sample1, sample2, min_obj_pixels=100, max_obj_n=10):

    sample1_images = _get_images(sample1)
    sample2_images = _get_images(sample2)

    sample1_aux_images = _get_aux_images(sample1)
    sample2_aux_images = _get_aux_images(sample2)
    ...
    all_aux_img = []
    all_mask = []
    for idx, (s1_img, s2_img, s1_aux_img, s2_aux_img, s1_label, s2_label) in enumerate(
            zip(sample1_images, sample2_images, sample1_aux_images, sample2_aux_images, sample1_labels, sample2_labels)):
        s2_fg = (s2_label > 0).float()
        s2_bg = 1 - s2_fg
        merged_img = s1_img * s2_bg + s2_img * s2_fg
        merged_aux_img = s1_aux_img * s2_bg + s2_aux_img * s2_fg
        merged_mask = s1_label * s2_bg.long() + (
            (s2_label + max_obj_n) * s2_fg.long())
        merged_mask = (merged_mask == obj_idx).float()
        if idx == 0:
        ...
        all_img.append(merged_img)
        all_aux_img.append(merged_aux_img)
        all_mask.append(merged_mask)
    ...
    sample = {
        ...
        'prev_img': all_img[1],
        'curr_img': all_img[2:],
        'ref_aux_img': all_aux_img[0],
        'prev_aux_img': all_aux_img[1],
        'curr_aux_img': all_aux_img[2:],
        'ref_label': all_mask[0],
        'prev_label': all_mask[1],
        'curr_label': all_mask[2:]
    }
...
class VOSTrain_aux(Dataset):
    ...
    def reverse_seq(self, imagelist, aux_imagelist, lablist):
        if np.random.randint(2) == 1:
            imagelist = imagelist[::-1]
            aux_imagelist = aux_imagelist[::-1]
            lablist = lablist[::-1]
        return imagelist, aux_imagelist, lablist
    ...
    def get_image_label(self, seqname, imagelist, aux_imagelist, lablist, index):
        image = cv2.imread(
            os.path.join(self.image_root, seqname, imagelist[index]))
        image = np.array(image, dtype=np.float32)

        aux_image = cv2.imread(
            os.path.join(self.aux_image_root, seqname, aux_imagelist[index]))
        aux_image = np.array(aux_image, dtype=np.float32)

        if self.rgb:
            image = image[:, :, [2, 1, 0]]
            aux_image = aux_image[:, :, [2, 1, 0]]

        label = Image.open(
            os.path.join(self.label_root, seqname, lablist[index]))
        label = np.array(label, dtype=np.uint8)

        return image, aux_image, label

    def sample_sequence(self, idx):
        idx = idx % len(self.seqs)
        seqname = self.seqs[idx]
        imagelist, aux_imagelist, lablist = self.imglistdic[seqname]
        frame_num = len(imagelist)
        if self.rand_reverse:
            imagelist, aux_imagelist, lablist = self.reverse_seq(imagelist, aux_imagelist, lablist)
        ...
        while (is_consistent is False and try_step < max_try):
            ...
            if self.enable_prev_frame:  # prev frame is randomly sampled
                # get prev frame
                prev_index = self.get_prev_index(lablist, total_gap)
                prev_image, prev_aux_image, prev_label = self.get_image_label(
                    seqname, imagelist, aux_imagelist, lablist, prev_index)
                prev_objs = list(np.unique(prev_label))

                # get curr frames
                curr_indices = self.get_curr_indices(lablist, prev_index,
                                                     curr_gaps)
                curr_images, curr_aux_images, curr_labels, curr_objs = [], [], [], []
                for curr_index in curr_indices:
                    curr_image, curr_aux_image, curr_label = self.get_image_label(
                        seqname, imagelist, aux_imagelist, lablist, curr_index)
                    c_objs = list(np.unique(curr_label))
                    curr_images.append(curr_image)
                    curr_aux_images.append(curr_aux_image)
                    curr_labels.append(curr_label)
                    curr_objs.extend(c_objs)
                ...
                while (ref_index > start_index and ref_index <= end_index
                    ...
                    _try_step += 1
                    ref_index = self.get_ref_index_v2(seqname, lablist)
                ref_image, ref_aux_image, ref_label = self.get_image_label(
                    seqname, imagelist, aux_imagelist, lablist, ref_index)
                ref_objs = list(np.unique(ref_label))
            else:  # prev frame is next to ref frame
                # get ref frame
                ref_index = self.get_ref_index_v2(seqname, lablist)
                ...
                objs = list(np.unique(curr_objs))
                prev_image, prev_aux_image, prev_label = curr_images[0], curr_aux_images[0], curr_labels[0]
                curr_images, curr_aux_images, curr_labels = curr_images[1:], curr_aux_images[1:], curr_labels[1:]
        ...
        sample = {
            'ref_img': ref_image,
            'ref_aux_img': ref_aux_image,
            'prev_img': prev_image,
            'prev_aux_img': prev_aux_image,
            'curr_img': curr_images,
            'curr_aux_img': curr_aux_images,
            'ref_label': ref_label,
            'prev_label': prev_label,
            'curr_label': curr_labels
        }
    ...
    def merge_sample(self, sample1, sample2, min_obj_pixels=100):
        return _merge_sample_with_aux(sample1, sample2, min_obj_pixels, self.max_obj_n)


class VisT300_Train(VOSTrain_aux):
    def __init__(self,
                 split=['train'],
                 root='./DAVIS',
                 transform=None,
        ...
        image_root = os.path.join(root, 'RGBImages')
        aux_image_root = os.path.join(root, 'ThermalImages')
        ...
        for seq_name in seq_names:
            ...
            aux_images = list(np.sort(os.listdir(os.path.join(aux_image_root, seq_name))))
            labels = list(np.sort(os.listdir(os.path.join(label_root, seq_name))))
            imglistdic[seq_name] = (images, aux_images, labels)

        super(VisT300_Train, self).__init__(image_root,
                                        aux_image_root,
                                        label_root,
                                        imglistdic,
                                        transform,
...
class VTUVA_Train(VOSTrain_aux):
    def __init__(self,
                 split=['train'],
                 root='./DAVIS',
                 transform=None,
        ...
        super(VTUVA_Train, self).__init__(image_root,
                                        aux_image_root,
                                        label_root,
                                        imglistdic,
                                        transform,
...
class ARKitTrack_Train(VOSTrain_aux):
    def __init__(self,
                 split=['train'],
                 root='./DAVIS',
                 transform=None,
        ...
        root = os.path.join(root, 'train')
        image_root = os.path.join(root, 'RGBImages')
        aux_image_root = os.path.join(root, 'DepthImages')
        label_root = os.path.join(root, 'Annotations')
        seq_names = os.listdir(image_root)
        ...
        super(ARKitTrack_Train, self).__init__(image_root,
                                        aux_image_root,
                                        label_root,
                                        imglistdic,
                                        transform,
...
class VisEvent_Train(VOSTrain_aux):
    def __init__(self,
                 split=['train'],
                 root='./DAVIS',
                 transform=None,
        ...
        root = os.path.join(root, 'train')
        image_root = os.path.join(root, 'RGBImages')
        aux_image_root = os.path.join(root, 'EventImages')
        label_root = os.path.join(root, 'Annotations')
        seq_names = os.listdir(image_root)
        ...
        super(VisEvent_Train, self).__init__(image_root,
                                        aux_image_root,
                                        label_root,
                                        imglistdic,
                                        transform,
