    ...
    def rescale(self, sample):
        ...
        for elem in sample.keys():
            ...
            if elem == 'prev_img' or elem == 'curr_img' or elem == 'ref_img' \
                or elem == 'prev_aux_img' or elem == 'curr_aux_img' or elem == 'ref_aux_img':
                flagval = cv2.INTER_CUBIC
            else:
                flagval = cv2.INTER_NEAREST

            if elem == 'curr_img' or elem == 'curr_aux_img' or elem == 'curr_label':
                new_tmp = []
                all_tmp = tmp
                for tmp in all_tmp:
                    tmp = cv2.resize(tmp,
    ...
    def __call__(self, sample):
        ...
        for elem in sample.keys():
            if 'meta' in elem or 'label' in elem:
                continue
            if elem == 'curr_img' or elem == 'curr_aux_img':
                new_tmp = []
