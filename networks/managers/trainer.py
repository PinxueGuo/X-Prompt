...
from dataloaders.train_datasets import VisT300_Train, VTUVA_Train, ARKitTrack_Train, VisEvent_Train
...
class Trainer(object):
    ...
    def set_learnable_and_frozen(self):
        # open LoRA, freeze others
        lora.mark_only_lora_as_trainable(self.model, bias='lora_only')
        # open some modules
        for name, param in self.model.named_parameters():
            if  'backbone' in name and 'lora' not in name:
                if 'prompt' in name:
        ...
        if self.rank == 0:
            ...
            for name, param in self.model.named_parameters():
                if param.requires_grad:
                    ...
                    learnable_param += param.numel()
                    print(name, param.shape)
                else:
                    total_param += param.numel()
            print('Total Param: {:.2f}M'.format(total_param / 1e6))
            print('Learnable Param: {:.2f}M'.format(learnable_param / 1e6))
            print('{:.2f}% of params are learnable.'.format(100.0*learnable_param/total_param))
        ...
        if cfg.TRAIN_RESUME:
            if self.rank == 0:
                try:
                    ...
                    ema_model, removed_dict = load_network(
                        self.model, ema_ckpt_dir, self.gpu, self.rank)
            ...
            if cfg.PRETRAIN_FULL:
                ...
                self.model, removed_dict = load_network(
                    self.model, cfg.PRETRAIN_MODEL, self.gpu, self.rank)
                if len(removed_dict) > 0:
    ...
    def prepare_dataset(self):
        ...
        if 'vist300' in cfg.DATASETS:
            train_vist300_dataset = VisT300_Train(
                root=cfg.DIR_VIST300,
                transform=composed_transforms,
                seq_len=cfg.DATA_SEQ_LEN,
                rand_gap=cfg.DATA_RANDOM_GAP_YTB,
                rand_reverse=cfg.DATA_RANDOM_REVERSE_SEQ,
                ...
                enable_prev_frame=self.enable_prev_frame,
                max_obj_n=cfg.MODEL_MAX_OBJ_NUM)
            train_datasets.append(train_vist300_dataset)

        if 'vtuva' in cfg.DATASETS:
            train_vtuva_dataset = VTUVA_Train(
                root=cfg.DIR_VTUVA,
                transform=composed_transforms,
                seq_len=cfg.DATA_SEQ_LEN,
                rand_gap=cfg.DATA_RANDOM_GAP_YTB,
                ...
                enable_prev_frame=self.enable_prev_frame,
                max_obj_n=cfg.MODEL_MAX_OBJ_NUM)
            train_datasets.append(train_vtuva_dataset)

        if 'arkittrack' in cfg.DATASETS:
            train_arkittrack_dataset = ARKitTrack_Train(
                root=cfg.DIR_ARKITTRACK,
                transform=composed_transforms,
                seq_len=cfg.DATA_SEQ_LEN,
                rand_gap=cfg.DATA_RANDOM_GAP_YTB,
                rand_reverse=cfg.DATA_RANDOM_REVERSE_SEQ,
                ...
                enable_prev_frame=self.enable_prev_frame,
                max_obj_n=cfg.MODEL_MAX_OBJ_NUM)
            train_datasets.append(train_arkittrack_dataset)

        if 'visevent' in cfg.DATASETS:
            train_visevent_dataset = VisEvent_Train(
                root=cfg.DIR_VISEVENT,
                transform=composed_transforms,
                seq_len=cfg.DATA_SEQ_LEN,
                rand_gap=cfg.DATA_RANDOM_GAP_YTB,
                rand_reverse=cfg.DATA_RANDOM_REVERSE_SEQ,
                ...
                enable_prev_frame=self.enable_prev_frame,
                max_obj_n=cfg.MODEL_MAX_OBJ_NUM)
            train_datasets.append(train_visevent_dataset)
    ...
    def sequential_training(self):
        ...
        while step < cfg.TRAIN_TOTAL_STEPS:
            ...
            for frame_idx, sample in enumerate(train_loader):
                ...
                ref_aux_imgs = sample['ref_aux_img']
                prev_aux_imgs = sample['prev_aux_img']
                curr_aux_imgs = sample['curr_aux_img']
                ...
                ref_aux_imgs = ref_aux_imgs.cuda(self.gpu, non_blocking=True)
                prev_aux_imgs = prev_aux_imgs.cuda(self.gpu, non_blocking=True)
                curr_aux_imgs = [
                    curr_aux_img.cuda(self.gpu, non_blocking=True)
                    for curr_aux_img in curr_aux_imgs
                ]
                ...
                all_frames = torch.cat([ref_imgs, prev_imgs] + curr_imgs,
                                       dim=0)
                all_aux_frames = torch.cat([ref_aux_imgs, prev_aux_imgs] + curr_aux_imgs,
                                             dim=0)
                ...
                # ---------- for mvp-vos exp ----------
                # ref_imgs = (ref_imgs + ref_aux_imgs)/2.
                # prev_imgs = (prev_imgs + prev_aux_imgs)/2.
                # curr_imgs = [(curr_img + curr_aux_img)/2. for curr_img, curr_aux_img in zip(curr_imgs, curr_aux_imgs)]
                # all_frames = (all_frames + all_aux_frames)/2.
                ref_imgs = torch.cat([ref_imgs, ref_aux_imgs], dim=1)
                prev_imgs = torch.cat([prev_imgs, prev_aux_imgs], dim=1)
                curr_imgs = [torch.cat([curr_img, curr_aux_img], dim=1) for curr_img, curr_aux_img in zip(curr_imgs, curr_aux_imgs)]
                all_frames = torch.cat([all_frames, all_aux_frames], dim=1)
                ...
                if self.rank == 0:
                    ...
                    if step % cfg.TRAIN_TBLOG_STEP == 0:
                        ...
                        if seq_len == 1:
                            self.process_log(ref_imgs[:,0:3], all_f[-2][:,0:3], all_f[-1][:,0:3],
