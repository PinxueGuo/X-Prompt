...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,cfg=None):
        ...
        self.linear_ID_K = lora.Linear(dim, self.num_heads, r=8)
        self.linear_ID_V = lora.Linear(dim, dim, r=8)
...
class ConvViT(nn.Module):
    ...
    def __init__(self, img_size=464, patch_size=[4, 2, 2], embed_dim=[256, 384, 768],
        ...
        self.prompt_fuser4 = nn.Linear(embed_dim[2]*2, embed_dim[2])
    ...
    def forward(self, template_patch, id_template, search, is_train, mem_k=None, mem_v=None, bs=None):
        ...
        if template_patch.shape[2]==search.shape[2]:
            ...
            rgb_template_patch = self.patch_embed1(rgb_template_patch)
            rgb_template_patch = self.pos_drop(rgb_template_patch)
            for blk in self.blocks1:
                rgb_template_patch = blk(rgb_template_patch)    # [b,d=256,H/4,W/4]
            ...
            rgb_template_patch = self.patch_embed2(fuse_template_patch)
            for blk in self.blocks2:
                rgb_template_patch= blk(rgb_template_patch)
            prompt_template_patch = self.prompt_patch_embed2(fuse_template_patch)
            ...
            rgb_template_patch = self.patch_embed3(fuse_template_patch)
            prompt_template_patch = self.prompt_patch_embed3(fuse_template_patch)
            ...
            rgb_template_patch = fuse_template_patch.flatten(2).permute(0, 2, 1)  # BCHW --> BNC
            rgb_template_patch = self.patch_embed4(rgb_template_patch)
            prompt_template_patch = fuse_template_patch.flatten(2).permute(0, 2, 1)  # BCHW --> BNC
            prompt_template_patch = self.prompt_patch_embed4(prompt_template_patch)
            fuse_template_patch = self.prompt_fuser4(
                torch.cat([rgb_template_patch, prompt_template_patch], dim=-1)
            )   # [b,HW,d]
        ...
        rgb_search = self.patch_embed2(fuse_search)
        for blk in self.blocks2:
            rgb_search = blk(rgb_search)
        prompt_search = self.prompt_patch_embed2(fuse_search)
        for blk in self.prompt_blocks2:
            prompt_search = blk(prompt_search)
        ...
        rgb_search = self.patch_embed3(fuse_search)
        prompt_search = self.prompt_patch_embed3(fuse_search)
        ...
        rgb_search = fuse_search.flatten(2).permute(0, 2, 1) #BCHW --> BNC
        rgb_search = self.patch_embed4(rgb_search)
        prompt_search = fuse_search.flatten(2).permute(0, 2, 1)  # BCHW --> BNC
        prompt_search = self.prompt_patch_embed4(prompt_search)
        fuse_search = self.prompt_fuser4(
            torch.cat([rgb_search, prompt_search], dim=-1)
        )
