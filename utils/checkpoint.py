...
def load_network(net, pretrained_dir, gpu, rank=0):
    print(rank)
    pretrained = torch.load(pretrained_dir,
                            map_location=torch.device("cuda:" + str(gpu)))
    ...
    modify_pretrain_weight_log = '########## Modify Pretrain Weight ##########\n'
    # resume OneVOS pretrained weight for PEFT
    if 'backbone.backbone.prompt_patch_embed1.proj.weight' not in pretrained_dict_update.keys():
        # copy path_embed_blocks for x_prompt
        for k, v in list(pretrained_dict.items()):  # Use list to avoid dictionary size change during iteration
            if 'patch_embed' in k:
                new_key = k.replace('patch_embed', 'prompt_patch_embed')
                pretrained_dict_update[new_key] = v
                modify_pretrain_weight_log += ' copy weight '+k+' --> '+new_key+'\n'
            elif 'blocks1' in k:
                new_key = k.replace('blocks1', 'prompt_blocks1')
                pretrained_dict_update[new_key] = v
                modify_pretrain_weight_log += ' copy weight '+k+' --> '+new_key+'\n'
            elif 'blocks2' in k:
                new_key = k.replace('blocks2', 'prompt_blocks2')
                pretrained_dict_update[new_key] = v
                modify_pretrain_weight_log += ' copy weight '+k+' --> '+new_key+'\n'
        # split linear_ID_KV to linear_ID_K and linear_ID_V
        for name, param in list(pretrained_dict.items()):
            if 'linear_ID_KV' in name:
                k_name = name.replace('linear_ID_KV', 'linear_ID_K')
                v_name = name.replace('linear_ID_KV', 'linear_ID_V')
                k_weight, v_weight = param.split([12, 768], dim=0)  # 12: num_heads, 768: dim_embed
                pretrained_dict_update[k_name] = k_weight
                pretrained_dict_update[v_name] = v_weight
                modify_pretrain_weight_log += ' split weight '+name+' --> '+k_name+' and '+v_name+'\n'

    model_dict.update(pretrained_dict_update)
    
    # # remove patch_embed1
    # model_dict = {k: v for k, v in model_dict.items() if 'backbone.backbone.patch_embed1' not in k}

    load_log = net.load_state_dict(model_dict, strict=False)
    del (pretrained)

    print(modify_pretrain_weight_log) if rank==0 else None
    print('########## Load_log: ########## \n', load_log) if rank==0 else None
